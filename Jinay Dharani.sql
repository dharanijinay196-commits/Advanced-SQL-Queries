...
from vehicle_info vi join vehicle_sales vs  on vi.vehicle_uid = vs.vehicle_uid
group by vi.fuel having avg(vs.selling_price)>600000;


#Q5. Find brands that have more than 50 vehicles listed.
select brand,count(*) as total_vehicles
from vehicle_info group by brand having count(*)>50;
...
from vehicle_info vi join vehicle_sales vs 
on vi.vehicle_uid = vs.vehicle_uid
group by vi.transmission having avg(vs.selling_price)>700000
and count(*)>=30;
...
SELECT vs.price_category,avg(vs.km_driven) AS avg_km,count(*) AS total_vehicles
...
select * from vehicle_sales where selling_price > 
(select avg(selling_price) from vehicle_sales);
...
from vehicle_info vi join vehicle_sales vs  
on vi.vehicle_uid = vs.vehicle_uid
group by vi.brand having avg(vs.selling_price)>
(select avg(vs.selling_price) from vehicle_info vi join vehicle_sales vs
on vi.vehicle_uid = vs.vehicle_uid where vi.brand = "Toyota");
...
from vehicle_info vi join vehicle_sales vs 
on vi.vehicle_uid = vs.vehicle_uid
group by vi.fuel having sum(vs.selling_price) >
(select sum(vs.selling_price)
from vehicle_info vi join vehicle_sales vs 
on vi.vehicle_uid = vs.vehicle_uid where vi.fuel = "petrol");